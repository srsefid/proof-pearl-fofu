...
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{assert}\ {\isacharparenleft}p{\isasymnoteq}{\isacharbrackleft}{\isacharbrackright}{\isacharparenright}{\isacharsemicolon}\isanewline
...
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{let}\ f{\isacharprime}\ {\isacharequal}\ NFlow{\isachardot}augmentingFlow\ c\ f\ p{\isacharsemicolon}\isanewline
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{let}\ f\ {\isacharequal}\ NFlow{\isachardot}augment\ c\ f\ f{\isacharprime}{\isacharsemicolon}\isanewline
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{assert}\ {\isacharparenleft}NFlow\ c\ s\ t\ f{\isacharparenright}{\isacharsemicolon}\isanewline
...
\isamarkuptrue%
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}find{\isacharunderscore}shortest{\isacharunderscore}augmenting{\isacharunderscore}spec\ f\ {\isasymequiv}\ \isainnerkeyword{assert}\ {\isacharparenleft}NFlow\ c\ s\ t\ f{\isacharparenright}\ {\isasymthen}\ \isanewline
...
\isamarkuptrue%
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}edka\ {\isasymequiv}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
...\isanewline
{\isacharparenleft}f{\isacharcomma}{\isacharunderscore}{\isacharparenright}\ {\isasymleftarrow}\ \isainnerkeyword{while}\isactrlsub T\isactrlbsup fofu{\isacharunderscore}invar\isactrlesup \isanewline
\isanewline...
}
...
\begin{frame}[t]{Assembling Overall Correctness proof}
  \begin{itemize}
   ...
   \item<3-> Also integrated with check for valid network
    \begin{itemize}
     \item Input is list of edges, source, and sink
    \end{itemize}
    \begin{onlyenv}<3>
      \vspace{1em}
...
\begin{frame}[fragile]{Benchmarking}
  \begin{itemize}
   \item Against Java version of Sedgewick et al., on random networks
    \begin{itemize}
     \item Sparse graphs (density=0.02): Java is (slightly) faster
     \item Dense graphs (density=0.25): We are (slightly) faster
