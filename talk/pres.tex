...
\begin{frame}{Residual Graph}
  {of Network and Flow}
  \begin{itemize}
   \item<1-> Flow that can be moved between nodes
    \begin{itemize}
     \item By \textcolor{parval}{increasing} or \textcolor{aparval}{taking back} flow
    \end{itemize}
   \item<2-> Augmenting path: s-t path in residual graph
...
\begin{frame}[t]{And Automatic Proofs}
  {}
  
  \begin{itemize}
   \item<1-> Cormen et al. also give more complicated proofs
   \item<2-> We sometimes chose to use more automatic proofs
    \begin{itemize}
     \item<3-> Using some simplifier setup
     \item<4-> And auxiliary statements
     \item<5-> We reduce the displayed proof's complexity
...
\isamarkuptrue%
\isacommand{theorem}\isamarkupfalse%
\ fofu{\isacharunderscore}partial{\isacharunderscore}correct{\isacharcolon}\ {\isachardoublequoteopen}fofu\ {\isasymle}\ {\isacharparenleft}\isainnerkeyword{{spec}}\ f{\isachardot}\ isMaxFlow\ f{\isacharparenright}{\isachardoublequoteclose}\isanewline
%
\isadelimproof
\ \ %
\endisadelimproof
...
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}fofu\ {\isasymequiv}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
\ \ \isainnerkeyword{let}\ f\isactrlsub {\isadigit{0}}\ {\isacharequal}\ {\isacharparenleft}{\isasymlambda}{\isacharunderscore}{\isachardot}\ {\isadigit{0}}{\isacharparenright}{\isacharsemicolon}\isanewline
\isanewline
\ \ {\isacharparenleft}f{\isacharcomma}{\isacharunderscore}{\isacharparenright}\ {\isasymleftarrow}\ \isainnerkeyword{while}\isactrlbsup \isawithcolor{blue}{fofu{\isacharunderscore}invar}\isactrlesup \isanewline
...
\ \ \ \ \ \ \isainnerkeyword{case}\ p\ of\ \isanewline
\ \ \ \ \ \ \ \ None\ {\isasymRightarrow}\ \isainnerkeyword{return}\ {\isacharparenleft}f{\isacharcomma}True{\isacharparenright}\isanewline
\ \ \ \ \ \ {\isacharbar}\ Some\ p\ {\isasymRightarrow}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
\ \ \ \ \ \ \ \ \ \ \isawithcolor{blue}{ \isainnerkeyword{assert}\ {\isacharparenleft}p{\isasymnoteq}{\isacharbrackleft}{\isacharbrackright}{\isacharparenright}{\isacharsemicolon}\isanewline
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{assert}\ {\isacharparenleft}NFlow{\isachardot}isAugmentingPath\ c\ s\ t\ f\ p{\isacharparenright}{\isacharsemicolon}}\isanewline
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{let}\ f\ {\isacharequal}\ NFlow{\isachardot}augment{\isacharunderscore}with{\isacharunderscore}path\ c\ f\ p{\isacharsemicolon}\isanewline
\ \ \ \ \ \ \ \ \ \ \isawithcolor{blue}{ \isainnerkeyword{assert}\ {\isacharparenleft}NFlow\ c\ s\ t\ f{\isacharparenright}{\isacharsemicolon}}\isanewline
\ \ \ \ \ \ \ \ \ \ \isainnerkeyword{return}\ {\isacharparenleft}f{\isacharcomma}\ False{\isacharparenright}\isanewline
...
\ \ \isainnerkeyword{assert}\ {\isacharparenleft}NFlow\ c\ s\ t\ f{\isacharparenright}\ {\isasymthen}\ \isanewline
\ \ {\isacharparenleft}\isainnerkeyword{selectp}\ p{\isachardot}\ Graph{\isachardot}isShortestPath\ {\isacharparenleft}residualGraph\ c\ f{\isacharparenright}\ s\ p\ t{\isacharparenright}{\isachardoublequoteclose}%
}
\def\snipsrefine{
\isamarkuptrue%
\isacommand{lemma}\isamarkupfalse%
...
\isamarkuptrue%
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}\isawithcolor{blue}{edka{\isacharunderscore}partial}\ {\isasymequiv}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
...
\isamarkuptrue%
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}edka{\isacharunderscore}partial\ {\isasymequiv}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
...\isanewline
{\isacharparenleft}f{\isacharcomma}{\isacharunderscore}{\isacharparenright}\ {\isasymleftarrow}\ \isainnerkeyword{while}\isactrlbsup fofu{\isacharunderscore}invar\isactrlesup \isanewline
\isanewline...
}
...
\isamarkuptrue%
\isacommand{definition}\isamarkupfalse%
\ {\isachardoublequoteopen}\isawithcolor{blue}{edka}\ {\isasymequiv}\ \isainnerkeyword{do}\ {\isacharbraceleft}\isanewline
...\isanewline
{\isacharparenleft}f{\isacharcomma}{\isacharunderscore}{\isacharparenright}\ {\isasymleftarrow}\ \isawithcolor{blue}{ \isainnerkeyword{while}\isactrlsub T}\isactrlbsup fofu{\isacharunderscore}invar\isactrlesup \isanewline
\isanewline...
}


\begin{frame}[t]{Total Correctness and Complexity}
  \begin{itemize}
   \item<1-> Next, we define a total correct version
    \begin{onlyenv}<1>
      \vspace{1em}
    ...
    \begin{onlyenv}<2>
      ...
      \isasnip\edkatotal
    \end{onlyenv}
   \item<3-> And show refinement
    \begin{onlyenv}<3>
      \vspace{1em}
      
      \isasnip\edkarefine
    \end{onlyenv}
   \item<4-> We also show $O(VE)$ bound on loop iterations
    \begin{itemize}
     \item Instrumenting the loop with a counter
    \end{itemize}
  \end{itemize}
\end{frame}
}
...
\ \ \ \ \ \ \ \ {\isacharless}{\isasymlambda}fi{\isachardot}\ {\isasymexistsA}f{\isachardot}\ is{\isacharunderscore}rflow\ N\ f\ fi\ {\isacharasterisk}\ {\isasymup}{\isacharparenleft}isMaxFlow\ f{\isacharparenright}{\isachargreater}\isactrlsub t{\isachardoublequoteclose}\isanewline
%
}
...
\ \ \ \ \ \ \ {\isacharasterisk}\ {\isacharparenleft}{\isasymexistsA}f{\isachardot}\ is{\isacharunderscore}rflow\ c\ s\ t\ N\ f\ cf\ {\isacharasterisk}\ {\isasymup}{\isacharparenleft}Network{\isachardot}isMaxFlow\ c\ s\ t\ f{\isacharparenright}{\isacharparenright}\isanewline
\ \ \ \ {\isachargreater}\isactrlsub t{\isachardoublequoteclose}%
}



\begin{frame}[t]{Assembling Overall Correctness Proof}
  \begin{itemize}
   \item<1-> Correctness statement
    \begin{itemize}
     \item As Hoare Triple using Separation Logic
    \end{itemize}
    \begin{onlyenv}<1>
      \vspace{1em}
      ...
      \isasnip\snipthm
    \end{onlyenv}
   \item<2-> Proof by transitivity
    \begin{onlyenv}<2>
      \vspace{1em}
      
      \isasnip\snipproof
    \end{onlyenv}
   \item<3-> Also integrated with check for valid network
    \begin{itemize}
     \item Input: list of edges, source node, sink node
    \end{itemize}
    \begin{onlyenv}<3>
      \vspace{1em}
