...
\section{Refinement to Executable Code}\label{sec:executable}
  ...
  implementations of the push-relabel algorithms~\cite{ChGo97,ICPC-notebook} that we use in our benchmarks (\cf Section~\ref{sec:benchmarks}) lack such checks, and silently report erroneous maximum flow values in case of overflow.}
...
\begin{center}    
  \begin{tikzpicture}[thick,scale=.75, every node/.style={scale=1}] %change the scales if you like to reduce the size
	...
	\nextgroupplot[
      ...
      ytick = {0.1,1,4,8,16},
      xticklabels = {$16k$, $19k$, $22k$, $25k$, $28k$}, 
