...
%   A first time to compute the bottleneck, and a second time to do the augmentation. 
  ...
  \section{Push-Relabel algorithms}\label{sec:push_relabel}
  In this section we present the formalization of Push-relabel algorithms by
  Goldberg(XXX) that build up on Karzanov's idea of a preflow ---a relaxation
  of the flow definition by allowing the sum of flows over incoming edges to be
  greater equal than the sum of flows over outgoing edges of each vertex of the
  network. Karzanov was the first to leave the augmenting path idea in research
  on finding the maximum-flow in networks. He came up with an $O(V^3)$
  maximum-flow algorithm by relaxing some of the invariants in Dinitz's
  algorithm (XXX Dinitz paper). Years later, Goldberg proposed the generic
  Push-relabel algorithm which shares some ideas with Karzanov's algorithm, yet
  it provides freedom for choosing the order of ``push'' and ``relabel''
  operations. The push–relabel algorithm is considered one of the most efficient
  maximum flow algorithms. The generic algorithm has a strongly polynomial
  $O(V^2 E)$ time complexity, while specific variants of the algorithms achieve
  even lower time complexities up to $O(V^2 \sqrt{E})$. In what follows we first
  overview the transition from Ford-Fulkerson method to Push-relabel algorithm,
  and then provide the formal explanation of Goldberg's generic algorithm.
  ...
  The Push-relabels algorithms maintain a preflow as well as a height-labeling
...
Goldberg distinguishes two types of pushes which are latered used for proving
...
\texttt{---Discuss correctness and termination proofs if required}

\texttt{---Corelate to the FIFO and gap heuristic implementations}
