...
Further refinement yields a verified implementation, which is only 2.5 times slower than an unverified  
...
\section{Refinement in Isabelle}
...
%         case p of None ⇒ ¬conn src dst | Some p ⇒ shortestPath src p dst)"
  ...
  Down to executable code:
    Implement things that where not yet specified/specified in a non-executable way:
      ...
      Residual graph: All operations of the algorithm can also be expressed as only modifying the residual graph. 
      Although we can compute the edges of the residual graph from the capacities and the current flow on the fly,
      this still requires 2 or 3 accesses to adjacency matrices per edge. 
      
      Thus, we refine the algorithm to directly operate on residual graphs. The shortest path specification and bottleneck computation are already 
