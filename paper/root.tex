...
\begin{abstract}
We present a formalization of two classical algorithms for computing the maximum flow in a network:
The Edmonds-Karp algorithm and Goldberg's Generic Push-Relabel algorithm. We prove the correctness and complexity of the algorithms.
...
Using stepwise refinement, we obtain the Fifo-Push-Relabel algorithm with gap heuristics. 
Further refinement yields verified efficient implementations of both, Edmonds-Karp and Fifo-Push-Relabel, 
which compare well to unverified reference implementations in Java.

\keywords{Maxflow Algorithm \and Edmonds-Karp Algorithm \and Push-Relabel Algorithm \and Formal Verification \and Isabelle/HOL \and Stepwise Refinement}
...
Karzanov~\cite{Ka74} was the first to propose an $O(V^3)$ algorithm based on the idea of \emph{preflows}. Based on Karzanov's ideas, Goldberg et al.\ proposed
the generic push-relabel algorithm, which consists of repeated application of two local operations, push and relabel, in any order.
...
which we then refine to Edmonds-Karp algorithm and finally to an efficient implementation. Similar, the generic push-relabel algorithm is refined to 
the fifo-push-relabel algorithm, which, in turn, is refined to an efficient implementation.
...
A \emph{preflow} $f$ on a network is a labeling of the edges with real values satisfying the following constraints: 1) \emph{Capacity constraint}: the flow on each edge is a non-negative value smaller or equal to the edge's capacity; 2) \emph{Non-generation constraint}: For all vertices except $s$ and $t$, the sum of flows over all incoming edges is not smaller than the sum of flows over all outgoing edges.
...
\section{The Generic Push-Relabel Algorithm}  
  We present our formalization of the generic push relabel algorithm of Goldberg et al.~\cite{GoTa88}. 
  ...
  By generalizing the labeling invariant to paths, we get that for each path $p$ from $u$ to $v$, we have $l(u) \le l(v) + |p|$.
  ...
  A node that has positive excess is called \emph{active}. Upon termination of the algorithm, there will be no active nodes. 
  Thus, the preflow is actually a flow, and as we have no $s$--$t$ paths, the min-cut-max-flow theorem implies that it is maximal.
  
  Initially, the height of all nodes but the source is $0$, and the outgoing edges of the source get the maximum possible flow, 
  and all other edges get no flow at all. Then, the algorithm repeats the following operations:
  \begin{description}
    \item[push] Send as much flow as possible over an edge $(u,v)$ with $l(u) = l(v)+1$. The maximum possible flow is constrained
      by the residual capacity of the edge, and the excess flow available at $u$.
    \item[relabel] Increase the height of an active (non-sink) node from which no push operation is possible. 
      The height is increased to the minimum value such that a push becomes possible.
  \end{description}
  The algorithm is generic in the sense that it leaves open the order in which the push and relabel operations on the different nodes and edges are performed.
  The algorithm terminates if no more operations apply. Then, there are no active nodes left, and the algorithm has constructed a maximum flow.
...
(to find the new height, we have to consider the heights of all adjacent nodes in the residual graph) To this end, we define 
\isai{cost PUSH == 1 | cost RELABEL == card(V)}. We prove the following theorem:
\begin{lstlisting}
...
\end{lstlisting}  
where \isai$(f_0,l_0)$ is the initial flow and height-labeling. Intuitively, the cost of any (partial) run \isai$p$ is bounded by $O(V^2E)$, and if the 
run is complete, \ie no more operations apply (\isai$(f,l)\<notin>Domain pr_lts$), the resulting flow is maximal. 

For the proof of this theorem, we follow the presentation of Cormen~\cite{CLRS09}. 
However, at several places, we trade precision of the constant for simplicity of the proof. 
Thus, our constant of $38$ is somewhat higher than what would result from Cormen's proof.

For the correctness argument, we show that the operations of the algorithm maintain a valid labeling, and that existence of an active node implies applicability of 
a push or relabel operation.
The complexity argument is more tricky: First, we show that the labels of each node remain smaller than $2|V|$. This bounds the number of relabel operations to $O(V^2)$.
For the push operations, we first define a push to be \emph{saturating}, if the excess at the source node of the push is not less than the capacity of the edge.
...
of saturating push operations. Finally, the number of non-saturating pushes is bounded by defining a potential function that is decreased by non-saturating pushes,
and increased by at most $O(V)$ by the other operations. The initial potential is $O(V)$, and with the already established bounds on the other operations we get a bound of
...
The simple structure of the generic algorithm allowed us to conveniently define it as labeled transition system, such that the above intuitive arguments could be formalized
as counting different operation types on execution paths of the algorithm. Moreover, the powerful Isabelle/HOL standard library, in particular the lemmas for summations
over finite sets, were very helpful. Anyway, the whole complexity proof is roughly 1000 lines of Isabelle text, and took a few men weeks to complete.

\section{Implementing the Push-Relabel Algorithm}
The generic algorithm does not specify the order in which the operations should be applied. 
Moreover, choosing the right ordering may further decrease the worst-case time complexity, and, more important, the practical efficiency of the algorithm.

We refine the generic push-relabel algorithm to different implementations. However, these have a more complicated algorithmic structure that
prevents a convenient specification as labeled transition system, and thus a complexity proof in the style of the generic algorithm.
Thus, we specify the algorithms using the Monadic Refinement Framework, and only prove correctness and termination 
(which we get for free, as the complexity bound for the generic algorithm is also valid for all implementations).
We leave it to future work to integrate complexity reasoning tools into the Refinement Framework and prove tighter complexity bounds for the specific implementations.

\subsection{Relabel-to-Front Algorithm}
A general idea of the implementation is to \emph{discharge} a node by repeatedly applying push and relabel operations to the same node, until it gets inactive.
Cormen et al.\ present the relabel to front algorithm, which scans through a list of all nodes, discharging every node as it is visited.
If the node gets relabeled during discharging, it is moved to the front of the list, and the scan of the list is restarted at the beginning of the list.
The algorithm terminates if it reaches the end of the list. 
During discharging a node $u$, the algorithm iterates over the list of all adjacent nodes $v$ in the network, and checks whether there is an edge $(u,v)$ in 
the residual graph. When the discharge operation stops without relabeling $u$, the current position in the list of adjacent nodes is stored, and 
the next discharge operations starts at exactly this position.
...
Following the outline by Cormen et al., and using the Isabelle Refinement Framework to conveniently describe the abstract algorithm,
...
In each iteration, it pulls a node from the FIFO, and repeatedly pushes flow out of it.
If no push operations are possible any more, but the node is still active, it is relabeled and appended to the FIFO.

The FIFO push relabel algorithm has several advantages: It's correctness is straightforward to prove, 
the only required additional invariant is that the queue contains exactly the active nodes. 
...
all nodes with $k<l(v)<|V|$ may be relabeled to $|V|+1$, preserving a valid labeling. In practice, this may drastically accelerate convergence of the algorithm,
as it prevents unnecessary pushes of flow towards the sink when the path to the sink is already blocked.
The correctness proof of this heuristics is straightforward from the definitions, and also the overall correctness proof 
is straightforward: It's size is about 400 lines of code, and it took roughly a day to complete.
...
\section{Refinement to Executable Code}\label{sec:executable}
  ...
  Moreover, some of the used operations require additional auxiliary data structures to be maintained. For example, 
  we maintain the current excess of each node explicitly, instead of summing up the flows of incoming and outgoing edges each time 
  ...
  For example, the residual graph is modeled as a function from edges to capacities on the abstract level. In the implementation, we want to use a two-dimensional array.
...
%   Using this to refine the Edmonds-Karp formalization yields a program that algorithmically specifies all major operations.
  ...
  Next, we observe that the Edmonds-Karp algorithm is phrased in terms of a flow, which is updated until it is maximal. 
  ...
  For an efficient implementation of the gap heuristics, we introduce an explicit map from height-values to then number of nodes with this height.
  Using this map, it is easy to detect when we relabel the last node of a specific height and the gap heuristics applies.
  ...
  For the discharge operation, it is straightforward to find the new label by iterating over the adjacent edges in the residual graph.
  ...
  For the push-to-front algorithm, we select the edges from a set of unprocessed edges which is maintained globally.
  In the concrete implementation, however, we fold over a list of edges. The structure of the resulting concrete algorithms is somewhat different from the structure
  of the abstract algorithm --- too different for the heuristics of our verification condition generator. Thus, we had to manually prove some rules specifically 
  tailored to the required structural changes.
  ...
  Similar, the relabel operation has to iterate over all successors to find the new minimal labeling, and the discharge operations iterate over all outgoing edges of a node.
  Although these iterations can be implemented on the edge labeling function by iterating over all nodes, this implementation tends to be inefficient for sparse graphs,
...
%   the BFS, is represented as a list of edges. 
  ...
  For the push-relabel algorithms, the map from nodes to the excess, and the height frequency map are implemented by arrays. 
  ...
  Similar, the list of the relabel to front algorithm is implemented as two lists, \emph{left} and \emph{right}, that contain the nodes left and right of the current position,
  the left list being stored in reverse order. Thus, advancing the position is implemented in constant time by popping the first element of the right list, and pushing 
  ...
  Also the FIFO queue could be implemented by a ring buffer based on an array, yielding true constant-time operations with less overhead.
...
%         ylabel = Execution time in seconds, 
    ...
    %start data for plotting
      ...
      \addplot table {
        ...
        5000 .457 
        5500 .522     
      }; \addlegendentry{prpu-Java2, sparse};
      ...
      \addplot table {
        ...
        5000 .084 
        5500 .085 
      }; \addlegendentry{prpu-Java1, sparse};
...
%         ylabel = Execution time in seconds, 
    ...
    %start data for plotting
      ...
      \addplot table {
        ...
        1400	0.059
        1450	0.069  
      }; \addlegendentry{prpu-Java2, dense};
      ...
      \addplot table {
        ...
        1400	0.062
        1450	0.066
      }; \addlegendentry{prpu-Java1, dense};
...
%         ylabel = Execution time in seconds, 
    ...
    %start data for plotting
      ...
      \addplot table {
        ...
        5000	745.695
        5500	661.721
      }; \addlegendentry{prpu-Java2, wash};
      ...
      \addplot table {
        ...
        5000	40.456
        5500	39.012
      }; \addlegendentry{prpu-Java1, wash};
...
%         ylabel = Execution time in seconds, 
    ...
    %start data for plotting
      ...
      \addplot table {
        ...
        1400	60.096
        1450	65.875
      }; \addlegendentry{edka-Java, acnet};
      ...
      \addplot table {
        ...
        1400	0.16
        1450	0.185
      }; \addlegendentry{prpu-Java2, acnet};
      ...
      \addplot table {
        ...
        1400	0.21
        1450	0.269
      }; \addlegendentry{prpu-Java1, acnet};
