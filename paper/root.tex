...
\section{Refinement to Executable Code}\label{sec:executable}
  ...
  The most complex problem for which we specified no algorithm is finding a shortest augmenting path.
  ...
  is independent of flow networks, and can be re-used to find shortest path in directed graphs also in other applications.
  
  Minor problems for which we had to develop algorithms are augmentation of the flow along the shortest path, and the relabel and discharge operations.
  While it is straightforward to find the height to relabel a node with by iterating over the outgoing edges, the discharge operation is a bit more tricky:
  Abstractly, we nondeterministically select a new edge to push, until we find no more edges.
  ...
  Another important optimization is to pre-compute an adjacency map of the network:
  ...
  The height frequency map and the maps from nodes to heights and excess values are implemented as arrays.
...
\section{Benchmarking}\label{sec:benchmark}
  ...
  We extract SML code for our verified Edmonds-Karp and FIFO push-relabel algorithms, and compile with MLton 20100608~\cite{MLton}.
  ...
  The results are displayed in Figure~\ref{fig:benchmark}, using a double-logarithmic scale.
  Note that we only measure the time required to run the algorithms themselves, excluding the time required for parsing the input or the network checker.
  ...
  even Edmonds-Karp algorithm is competitive to the simple FIFO push-relabel algorithms, while the highly optimized one shows no weakness on these networks.
...
\begin{center}    
  \begin{tikzpicture}[thick,scale=.75, every node/.style={scale=1}] %change the scales if you like to reduce the size
    ...
    % legend information
      ...
      \addlegendimage{fill=gray, mark=square*}
      \addlegendimage{dashed, every mark/.append style={solid, fill=gray}, mark=*}
      \addlegendimage{fill=gray, mark=o}
      \addlegendimage{fill=gray, mark=otimes*}
    ...
    % end legend information
		...
		%start data for plotting
		  ...
		  \addplot+[dashed, every mark/.append style={solid, fill=gray},mark=*] table {
		    1000 .024   
		    1500 .054   
		    2000 .103  
		    2500 .399  
