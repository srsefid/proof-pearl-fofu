...
To simplify reasoning about the maximum flow problem, we assume that our network satisfies some additional constraints: 1) The source only has outgoing edges while the sink only has incoming edges; 2) If the flow network contains an edge $(u, v)$ then there is no \emph{parallel edge} $(v, u)$ in the reverse direction\footnote{With $u=v$, this also implies that there are no self loops.}; and 3) every vertex of the flow network must be on a path from $s$ to $t$. Note that any network can be transformed to a network with the aforementioned properties and the same maximum flow~\cite{CLRS09}.
...
Each edge is labeled by the amount of flow that can be effectively moved along this edge, by either increasing or decreasing the flow on a network edge.
...
Networks are based on graphs, and add the source and sink node, as well as the network assumptions:
\begin{lstlisting}
...
If either \isai{m = fail}, or \isai{f} may fail for a result in \isai{m}, the bind operation fails.
...
which relates concrete with abstract results: Given a relation \isai{R}, \isai{\<Down>R m} is the largest set of concrete results that are related to an 
...
\section{Benchmarking}\label{sec:benchmark}
  ...
  \begin{figure}[!h]
  \begin{tikzpicture}[thick,scale=.7, every node/.style={scale=1}] %change the scales if you like to reduce the size
    \begin{axis}[
  ...
  \begin{tikzpicture}[thick,scale=.7, every node/.style={scale=1}] %change the scales if you like to reduce the size
    \begin{axis}[
      %title={Benchmark of the Edmonds-Karp algorithm},
        ...
        ytick = {0,100,1000,10000},
        xticklabels = {$1000$, $1100$, $1200$, $1300$, $1400$}, %corresponds to the first column in {SML, Sparse}
%         yticklabels = {$0.2$, $1.4$, $4.9$, $9.2$, $23.6$}, %corresponds to the second column in {SML, Sparse}
        smooth,
        cycle list name = black white,
        legend style = { 
...
\section{Conclusion}\label{sec:concl}
  ...
  As far as we could understand the Mizar proof script, it formalizes an algorithm roughly equivalent to our abstract version of the Ford-Fulkerson method.
