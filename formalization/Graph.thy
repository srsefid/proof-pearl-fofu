...
begin
  ...
  begin
    ...
    lemma sum_outgoing_alt: "\<lbrakk>finite V; \<forall>e. 0 \<le> g e \<and> g e \<le> c e\<rbrakk> \<Longrightarrow>
      \<forall>v \<in> V. (\<Sum>e \<in> \<delta>\<^sup>- v. g e) = (\<Sum>u \<in> V. g (v, u))"
      proof -
    ...
    lemma sum_incoming_alt: "\<lbrakk>finite V; \<forall>e. 0 \<le> g e \<and> g e \<le> c e\<rbrakk> \<Longrightarrow>
      \<forall>v \<in> V. (\<Sum>e \<in> \<delta>\<^sup>+ v. g e) = (\<Sum>u \<in> V. g (u, v))"
      proof -
        assume asm1: "finite V"
        assume asm2: "\<forall>e. 0 \<le> g e \<and> g e \<le> c e"
        {
  ...
  context Graph
  begin 
    lemma isPath_append: "\<langle>\<leadsto> u, p1 @ p2, v\<rangle> \<longleftrightarrow> (\<exists>w. \<langle>\<leadsto> u, p1, w\<rangle> \<and> \<langle>\<leadsto> w, p2, v\<rangle>)"  
      by (induction p1 arbitrary: u) auto 
      
    lemma isPath_head: "\<langle>\<leadsto> u, e # p, v\<rangle> \<longleftrightarrow> fst e = u \<and> e \<in> E \<and> \<langle>\<leadsto> snd e, p, v\<rangle>"
      by (cases e) auto

    lemma isPath_head2: "\<langle>\<leadsto> u, e # p, v\<rangle> \<Longrightarrow> (p = [] \<or> (p \<noteq> [] \<and> fst (hd p) = snd e))"
      by (metis Graph.isPath_head list.collapse)
      
    lemma isPath_tail: "\<langle>\<leadsto> u, p @ [e], v\<rangle> \<longleftrightarrow> \<langle>\<leadsto> u, p, fst e\<rangle> \<and> e \<in> E \<and> snd e = v"
      by (induction p) (auto simp: isPath_append isPath_head)
    
    lemma isPath_tail2: "\<langle>\<leadsto> u, p @ [e], v\<rangle> \<Longrightarrow> (p = [] \<or> (p \<noteq> [] \<and> snd (last p) = fst e))"
      by (metis Graph.isPath_tail append_butlast_last_id)
      
    lemma isPath_edgeset: "\<lbrakk>\<langle>\<leadsto> u, p, v\<rangle>; (x, y) \<in> set p\<rbrakk> \<Longrightarrow> (x, y) \<in> E"
      using E_def 
      by (metis (mono_tags, lifting) isPath_head isPath_append in_set_conv_decomp_first)
      
    lemma isPath_alt: "\<langle>\<leadsto> u, p, v\<rangle> \<Longrightarrow> (u, v) \<in> E\<^sup>*"
    ...
    lemma isPath_alt_rtc1: "(u, v) \<in> E\<^sup>* \<Longrightarrow> (\<exists>p. \<langle>\<leadsto> u, p, v\<rangle>)"
      ...
      next
        case (rtrancl_into_rtrancl u u' v)
          ...
          ultimately have "\<langle>\<leadsto> u, p1 @ [(u', v)] , v\<rangle>" using isPath_tail by simp
          thus ?case by blast
      qed
    
    lemma isPath_alt_rtc2: "(v, u) \<in> (E\<inverse>)\<^sup>* \<Longrightarrow> (\<exists>p. \<langle>\<leadsto> u, p, v\<rangle>)"
      proof (induction rule: rtrancl.induct)
        case (rtrancl_refl a)
          have "isPath a [] a" by simp
          thus ?case by blast
      next
        case (rtrancl_into_rtrancl v u' u)
          ...
          ultimately have "\<langle>\<leadsto> u, (u, u') # p1, v\<rangle>" using isPath_head by simp
          thus ?case by blast
      qed  
      
    lemma isPath_ex_edge1: "\<lbrakk>\<langle>\<leadsto> u, p, v\<rangle>; (u1, v1) \<in> set p; u1 \<noteq> u\<rbrakk> \<Longrightarrow> \<exists> u2. (u2, u1) \<in> set p"
      proof -
        assume asm1: "\<langle>\<leadsto> u, p, v\<rangle>"
    ...
    lemma isPath_ex_edge2: "\<lbrakk>\<langle>\<leadsto> u, p, v\<rangle>; (u1, v1) \<in> set p; v1 \<noteq> v\<rbrakk> \<Longrightarrow> \<exists> v2. (v1, v2) \<in> set p"
      proof -
        assume asm1: "\<langle>\<leadsto> u, p, v\<rangle>"
        assume asm2: "(u1, v1) \<in> set p"
    ...
    lemma reachable_ss_V: "s \<in> V \<Longrightarrow> \<langle>\<star> s\<rangle> \<subseteq> V"
      proof
        ...
        then obtain p where "x \<in> {v. isPath s p v}"
          unfolding reachableNodes_def isReachable_def by blast
        thus "x \<in> V" using asm
          by (induction p arbitrary: s) (auto simp: isPath_head E_def V_alt) 
      qed

    lemma isReachable_refl[simp]: "\<langle>v \<leadsto> v\<rangle>" 
      unfolding isReachable_def by (force intro: exI[where x="[]"])

    lemma isReachable_edgeRtc: "\<langle>u \<leadsto> v\<rangle> \<longleftrightarrow> (u, v) \<in> E\<^sup>*"  
    proof
  ...
  context Graph
  begin          
    lemma isSPath_pathLE: "\<langle>\<leadsto> u, p, v\<rangle> \<Longrightarrow> \<exists>p'. \<langle>\<Rightarrow> u, p', v\<rangle>"
      proof (induction p arbitrary: u)
        case Nil
    ...
    lemma isSPath_no_selfloop: "\<langle>\<Rightarrow> u, p, v\<rangle> \<Longrightarrow> (u1, u1) \<notin> set p"
      proof (rule ccontr)
    ...
    lemma isSPath_sg_outgoing: "\<lbrakk>\<langle>\<Rightarrow> u, p, v\<rangle>; (u1, v1) \<in> set p; v1 \<noteq> v2\<rbrakk> \<Longrightarrow> (u1, v2) \<notin> set p"
      proof -
    ...
    lemma isSPath_sg_incoming: "\<lbrakk>\<langle>\<Rightarrow> u, p, v\<rangle>; (u1, v1) \<in> set p; u1 \<noteq> u2\<rbrakk> \<Longrightarrow> (u2, v1) \<notin> set p"
      proof -
        assume asm1: "isSimplePath u p v"
        assume asm2: "(u1, v1) \<in> set p"
    ...
    lemma isSPath_no_returning: "\<lbrakk>\<langle>\<Rightarrow> u, p, v\<rangle>; (u1, v1) \<in> set p\<rbrakk> \<Longrightarrow>
    ...
    lemma isSPath_nt_parallel: "\<langle>\<Rightarrow> u, p, v\<rangle> \<Longrightarrow> (\<forall>(u, v) \<in> set p. (v, u) \<notin> set p)"
      proof -
        assume asm: "isSimplePath u p v"
        {
          fix x y
    ...
    lemma isSPath_distinct: "\<langle>\<Rightarrow> u, p, v\<rangle> \<Longrightarrow> distinct p"
      proof (rule ccontr)
        assume asm: "isSimplePath u p v"
