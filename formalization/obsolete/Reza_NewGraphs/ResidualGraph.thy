...
begin
  ...
  definition residualGraph :: "graph \<Rightarrow> flow \<Rightarrow> graph"
    ("\<langle>\<C>\<^sub>\<f>/ _,/ _\<rangle>" 1000)
  where "\<langle>\<C>\<^sub>\<f> c, f\<rangle> \<equiv> \<lambda>(u, v).
    if (u, v) \<in> \<lbrace>c \<parallel>\<^sub>G E\<rbrace> then
      c (u, v) - f (u, v)
    else if (v, u) \<in> \<lbrace>c \<parallel>\<^sub>G E\<rbrace> then
      f (v, u)
    else
      0"
  (*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*)
  ...
  context NFlow
  begin
    abbreviation "cf \<equiv> \<langle>\<C>\<^sub>\<f> c, f\<rangle>"
    
    definition isAugmenting :: "path \<Rightarrow> bool"
      ("\<langle>\<Rightarrow>\<^sup>A/ _\<rangle>" 1000)
    where "\<langle>\<Rightarrow>\<^sup>A p\<rangle> \<equiv> \<lbrace>cf \<parallel>\<^sub>G \<langle>\<Rightarrow> s, p ,t\<rangle>\<rbrace>"
      
    definition bottleNeck :: "path \<Rightarrow> capacity"
      ("\<langle>\<nabla>/ _\<rangle>" 1000)
    where "\<langle>\<nabla> p\<rangle> \<equiv> Min {cf e | e. e \<in> set p}"
      
    definition augmentingFlow :: "path \<Rightarrow> flow"
      ("\<langle>\<F>\<^sub>\<p>/ _\<rangle>" 1000)
    where "\<langle>\<F>\<^sub>\<p> p\<rangle> \<equiv> \<lambda>(u, v).
      if (u, v) \<in> (set p) then
        \<langle>\<nabla> p\<rangle>
      else
        0"
  end
  ...
  context NFlow
  begin
    lemma resV_netV : "\<lbrace>cf \<parallel>\<^sub>G V\<rbrace> = V"
      proof
    ...
    lemma resE_positive: "e \<in> \<lbrace>cf \<parallel>\<^sub>G E\<rbrace> \<Longrightarrow> cf e > 0"
      proof -
        assume asm: "e \<in> \<lbrace>cf \<parallel>\<^sub>G E\<rbrace>"
  ...
  context NFlow
  begin
    lemma bottleNeck_gzero: "\<lbrace>cf \<parallel>\<^sub>G \<langle>\<leadsto> s, p, t\<rangle>\<rbrace> \<Longrightarrow> \<langle>\<nabla> p\<rangle> > 0"
      proof -
    ...
    lemma augFlow_node_card: "\<langle>\<Rightarrow>\<^sup>A p\<rangle> \<Longrightarrow> \<forall> v \<in> \<lbrace>cf \<parallel>\<^sub>G V\<rbrace> - {s, t}.
      card {u |u. u \<in> \<lbrace>cf \<parallel>\<^sub>G V\<rbrace> \<and> (u, v) \<in> set p} = card {u |u. u \<in> \<lbrace>cf \<parallel>\<^sub>G V\<rbrace> \<and> (v, u) \<in> set p}"
    ...
    lemma augFlow_resFlow: "\<langle>\<Rightarrow>\<^sup>A p\<rangle> \<Longrightarrow> Flow cf s t \<langle>\<F>\<^sub>\<p> p\<rangle>"
      proof -
        ...
        {
          fix e
          have "0 \<le> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<and> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<le> cf e"
            proof (cases "e \<in> set p") 
              case True
                ...
                moreover have "0 < \<langle>\<nabla> p\<rangle>" using asm bottleNeck_gzero[of p] by simp 
          ...
          have "(\<Sum>e \<in> Graph.incoming cf v. (augmentingFlow p) e) =
            ...
            proof -
              ...
              let ?SUM = "\<lambda>s f. \<Sum>e \<in> s. f e"
              {
                ...
                have f2: "\<forall>e. 0 \<le> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<and> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<le> cf e" using fct1 resE_nonNegative by auto
              ...
              moreover {
                have f1: "finite (Graph.V cf)" using resV_netV finite_V by auto
                have f2: "\<forall>e. 0 \<le> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<and>(augmentingFlow p) e \<le> cf e" using fct1 by auto
    ...
    lemma augFlow_val: "\<langle>\<Rightarrow>\<^sup>A p\<rangle> \<Longrightarrow> \<lbrace>cf, s \<parallel>\<^sub>F |\<langle>\<F>\<^sub>\<p> p\<rangle>|\<rbrace> = \<langle>\<nabla> p\<rangle>"
      proof -
        assume asm: "isAugmenting p"
        {
        ...
        show ?thesis
          proof -
            ...
            moreover {
              have f1: "finite (Graph.V cf)" using resV_netV finite_V by auto
              have f2: "\<forall>e. 0 \<le> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<and> \<langle>\<F>\<^sub>\<p> p\<rangle> e \<le> cf e" using fct1 Graph.Flow_def by auto
              note Graph.sum_outgoing_alt[OF f1 f2]
            ...
            ultimately have "Flow.val cf s (augmentingFlow p) =
              (\<Sum>y\<in>{n. n < card ?S_OP}. \<langle>\<nabla> p\<rangle>) - (\<Sum>y\<in>{n. n < card ?S_IP}. \<langle>\<nabla> p\<rangle>)" by auto
            moreover {
