...
begin
  ...
  locale Graph = 
    fixes c :: graph
  begin
    definition "E \<equiv> {(u, v). c (u, v) \<noteq> 0}"
    
    definition "V \<equiv> {u. \<exists>v. (u, v) \<in> E \<or> (v, u) \<in> E}"
    lemma V_alt: "V = fst`E \<union> snd`E" unfolding V_def by force
    
    definition "incoming v \<equiv> {(u, v) | u. (u, v) \<in> E}"
    definition "outgoing v \<equiv> {(v, u) | u. (v, u) \<in> E}"
    definition "delta v \<equiv> incoming v \<union> outgoing v"
    
    definition "incoming' k \<equiv> {(u, v) | u v. u \<notin> k \<and> v \<in> k \<and> (u, v) \<in> E}"
    definition "outgoing' k \<equiv> {(v, u) | u v. u \<notin> k \<and> v \<in> k \<and> (v, u) \<in> E}"    

  end

  context Graph begin
    lemma succ_in_V: "E``{v} \<subseteq> V"  
      by (auto simp: V_def)
  
    lemma succ_in_V': 
      "(u,v)\<in>E \<Longrightarrow> u\<in>V"
      "(u,v)\<in>E \<Longrightarrow> v\<in>V"
      by (auto simp: V_def)
  end
  ...
  context Graph
  begin
    fun isPath :: "node \<Rightarrow> path \<Rightarrow> node \<Rightarrow> bool" where
      "isPath u [] v \<longleftrightarrow> u = v"
    | "isPath u ((s, t) # p) v \<longleftrightarrow> u = s \<and> (s, t) \<in> E \<and> isPath t p v"
    ...
    lemma isPath_edgeset: "isPath u p v \<Longrightarrow> e \<in> set p \<Longrightarrow> e \<in> E"
      using E_def by (metis (mono_tags, lifting) isPath_head isPath_append
      in_set_conv_decomp_first)
      
    lemma isPath_alt: "isPath u p v \<Longrightarrow> (u, v) \<in> E\<^sup>*"
    ...
    lemma isPath_alt_rtc1: "(u, v) \<in> E\<^sup>* \<Longrightarrow> (\<exists>p. isPath u p v)"
    ...
    lemma isPath_alt_rtc2: "(v, u) \<in> (E\<inverse>)\<^sup>* \<Longrightarrow> (\<exists>p. isPath u p v)"
      proof (induction rule: rtrancl.induct)
        case (rtrancl_refl a)
          have "isPath a [] a" by simp
          thus ?case by blast
      next
        case (rtrancl_into_rtrancl v u' u)
          then obtain p1 where "isPath u' p1 v" by blast
          moreover have "(u, u')\<in>E" using rtrancl_into_rtrancl.hyps(2) by simp
          ultimately have "isPath u ((u, u') # p1) v" using isPath_head by simp
          thus ?case by blast
      qed  
      
    lemma isPath_ex_edge1: "isPath u p v \<Longrightarrow> (u1, v1) \<in> set p \<Longrightarrow> u1 \<noteq> u \<Longrightarrow> \<exists> u2. (u2, u1) \<in> set p"
      proof -
    ...
    lemma isPath_ex_edge2: "isPath u p v \<Longrightarrow> (u1, v1) \<in> set p \<Longrightarrow> v1 \<noteq> v \<Longrightarrow> \<exists> v2. (v1, v2) \<in> set p"
      proof -
        assume asm1: "isPath u p v"
        assume asm2: "(u1, v1) \<in> set p"
  ...
  context Graph
  begin
    fun pathVertices :: "node \<Rightarrow> path \<Rightarrow> node list" where
      "pathVertices u [] = [u]"
  ...
  context Graph
  begin    
    definition isSimplePath :: "node \<Rightarrow> path \<Rightarrow> node \<Rightarrow> bool" where
      "isSimplePath u p v \<equiv> isPath u p v \<and> distinct (pathVertices u p)"
      

    lemma isSimplePath_empty[simp]: "isSimplePath u [] u"
      by (auto simp: isSimplePath_def)

    lemma isSimplePath_prepend: 
      assumes "(u,v)\<in>E" "u\<notin>set (pathVertices v p)" "isSimplePath v p w"
      shows "isSimplePath u ((u,v)#p) w"
      using assms
      apply (auto simp: isSimplePath_def)
      done      
    ...
    lemma isSPath_sg_outgoing: "isSimplePath u p v \<Longrightarrow> (u1, v1) \<in> set p \<Longrightarrow> v1 \<noteq> v2 \<Longrightarrow> 
      (u1, v2) \<notin> set p"
      proof -
    ...
    lemma isSPath_sg_incoming: "isSimplePath u p v \<Longrightarrow> (u1, v1) \<in> set p \<Longrightarrow> u1 \<noteq> u2 \<Longrightarrow> 
      (u2, v1) \<notin> set p"
      proof -
        assume asm1: "isSimplePath u p v"
        assume asm2: "(u1, v1) \<in> set p"
    ...
    lemma isSPath_no_returning: "isSimplePath u p v \<Longrightarrow> (u1, v1) \<in> set p \<Longrightarrow>
  ...
  context Graph
  begin
    definition "isReachable u v \<equiv> \<exists>p. isPath u p v"
    
    definition "reachable u \<equiv> {v. isReachable  u v}"
    
    lemma reachable_ss_V: "s \<in> V \<Longrightarrow> reachable s \<subseteq> V"
      proof
        assume asm: "s \<in> V"
        fix x
        assume "x \<in> reachable s"
        then obtain p where "x \<in> {v. isPath s p v}"
          unfolding reachable_def isReachable_def by blast
        thus "x \<in> V" using asm
          by (induction p arbitrary: s) (auto simp: isPath_head E_def V_alt) 
      qed

    lemma isReachable_refl[simp]: "isReachable v v" 
      unfolding isReachable_def by (force intro: exI[where x="[]"])

    lemma isReachable_alt: "isReachable u v \<longleftrightarrow> (u,v)\<in>E\<^sup>*"  
    proof
      assume "isReachable u v"
      then obtain p where "isPath u p v" by (auto simp add: isReachable_def)
      thus "(u,v)\<in>E\<^sup>*"
    ...
    next
      assume "(u,v)\<in>E\<^sup>*"
      thus "isReachable u v"
        proof (induction rule: converse_rtrancl_induct)
          case (step y z)
            from step.IH obtain p where "isPath z p v" by (auto simp add: isReachable_def)
            moreover from step.hyps have "((y,z)\<in>E)" by (simp)
            ultimately have "isPath y ((y,z)#p) v" by (auto simp: isPath_head)
            thus ?case unfolding isReachable_def by blast
        qed simp
    qed    

    lemma reachable_alt: "reachable u = E\<^sup>*``{u}"
      unfolding reachable_def isReachable_alt by auto

    lemma isReachableV: "\<lbrakk>isReachable u v\<rbrakk> \<Longrightarrow> v\<in>V \<longleftrightarrow> u\<in>V"
      apply (auto simp: isReachable_def)
      apply (case_tac p; auto intro: succ_in_V') []
      apply (case_tac p rule: rev_cases; auto simp: isPath_append intro: succ_in_V') []
      done
  ...
  locale Flow = Graph +
    fixes s t :: node
    fixes f :: flow
    assumes st_nodes[simp, intro!]: "s\<in>V" "t\<in>V"
    assumes capacity_cons: "\<forall>e. f e \<le> c e"
    assumes conservation_const: "\<forall>v \<in> V - {s, t}. (\<Sum>e \<in> incoming v. f e) = (\<Sum>e \<in> outgoing v. f e)"
  begin
    definition "val \<equiv> (\<Sum>e \<in> outgoing s. f e) - (\<Sum>e \<in> incoming s. f e)"
  end
  
  context Graph
  begin
   lemma sum_outgoing_alt: "finite V \<Longrightarrow> \<forall>e. g e \<le> c e \<Longrightarrow>
      \<forall>v \<in> V. (\<Sum>e \<in> outgoing v. g e) = (\<Sum>u \<in> {u | u. u \<in> V}. g (v, u))"
      proof -
        assume asm1: "finite V"
        assume asm2: "\<forall>e. g e \<le> c e"
        {
          fix v
          assume "v \<in> V"
          {
            then have "{(v, u) | u. u \<in> V} \<subseteq> V \<times> V" by blast
            then have fct1: "finite {(v, u) | u. u \<in> V}" using asm1 finite_subset by simp
            have "{(v, u) | u. u \<in> V \<and> (v, u) \<in> E} \<subseteq> {(v, u) | u. u \<in> V}" by blast
            then have f1:"finite {(v, u) | u. u \<in> V \<and> (v, u) \<in> E}" using fct1 finite_subset by auto
            have "{(v, u) | u. u \<in> V \<and> (v, u) \<notin> E} \<subseteq> {(v, u) | u. u \<in> V}" by blast
            then have f2:"finite {(v, u) | u. u \<in> V \<and> (v, u) \<notin> E}" using fct1 finite_subset by auto
              ...
              proof -
                {
                  ...
                  have "g e = 0"
                    proof (rule ccontr)
                      assume "\<not> g e = 0"
                      then have "c e \<noteq> 0" using asm2 by (metis le_0_eq)
    ...
    lemma sum_incoming_alt: "finite V \<Longrightarrow> \<forall>e. g e \<le> c e \<Longrightarrow>
      \<forall>v \<in> V. (\<Sum>e \<in> incoming v. g e) = (\<Sum>u \<in> {u | u. u \<in> V}. g (u, v))"
      proof -
        assume asm1: "finite V"
        assume asm2: "\<forall>e. g e \<le> c e"
        {
          ...
          assume "v \<in> V"
          {
            ...
            then have fct1: "finite {(u, v) | u. u \<in> V}" using asm1 finite_subset by simp
            have "{(u, v) | u. u \<in> V \<and> (u, v) \<in> E} \<subseteq> {(u, v) | u. u \<in> V}" by blast
            then have f1:"finite {(u, v) | u. u \<in> V \<and> (u, v) \<in> E}" using fct1 finite_subset by auto
            have "{(u, v) | u. u \<in> V \<and> (u, v) \<notin> E} \<subseteq> {(u, v) | u. u \<in> V}" by blast
            then have f2:"finite {(u, v) | u. u \<in> V \<and> (u, v) \<notin> E}" using fct1 finite_subset by auto
