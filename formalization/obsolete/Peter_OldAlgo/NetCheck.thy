...
begin
  ...
  definition ln_invar :: "edge_list \<Rightarrow> bool" where 
    "ln_invar el \<equiv> 
      distinct (map (\<lambda>(u, v, _). (u,v)) el) \<and> (\<forall>u v. (u, v, 0) \<notin> set el) 
    "
  ...
  lemma ln_\<alpha>_imp_in_set: "\<lbrakk>ln_\<alpha> el (u,v)>(0::capacity)\<rbrakk> \<Longrightarrow> (u,v,ln_\<alpha> el (u,v))\<in>set el"
  ...
  fun read :: "(nat \<times> nat \<times> nat) list \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow>
      ...
      case (Cons e es)
          ...
          
        {
          ...
          have "ln_invar (e # es)"
            proof (rule ccontr)
              assume "\<not> ?thesis"
              have f1: "\<forall>u v. (u, v, 0) \<notin> set (e # es)" using `ln_invar es` fct0 obt1
                  ...
                  case False
                    ...
                    obtain u1' v1' where "a = (u1', v1')" apply (cases a) by auto
                    {
                      have "(ln_\<alpha> (e # es)) (u1', v1') = (ln_\<alpha> es) (u1', v1')"
                        proof (cases "\<exists> c1'. (u1', v1', c1') \<in> set (e # es) \<and> c1' > 0")
                          case True
                ...
                proof (cases "c' (u1, v1) = 0 \<and> c' (v1, u1) = 0")
                  case True
                    ...
                    moreover have "v1 \<notin> set (ps' a)"
                      proof (rule ccontr)
                        assume "\<not> ?thesis"
                        then have "c' (a, v1) > 0 \<or> c' (v1, a) > 0"
                ...
                proof (cases "c' (u1, v1) = 0 \<and> c' (v1, u1) = 0")
                  case True
                    ...
                    moreover have "u1 \<notin> set (ps' a)"
                      proof (rule ccontr)
                        assume "\<not> ?thesis"
                        then have "c' (u1, a) > 0 \<or> c' (a, u1) > 0"
                          using fct unfolding Graph.E_def by auto
                        thus "False" using True `a = v1` by simp
                      qed
                    ultimately show ?thesis using `a = v1` fct by auto
  ...
  lemma read_correct2: "read el s t = None \<Longrightarrow> \<not>ln_invar el \<or> (\<exists>u c. (u, u, c) \<in> set el \<and> c > 0) \<or> 
    (\<exists>u c. (u, s, c) \<in> set el \<and> c > 0) \<or> (\<exists>u c. (t, u, c) \<in> set el \<and> c > 0) \<or>
    (\<exists>u v c1 c2. (u, v, c1) \<in> set el \<and> (v, u, c2) \<in> set el \<and> c1 > 0 \<and> c2 > 0)"
    proof (induction el)
      case Nil
        thus ?case by auto
    next
      case (Cons e el)
        thus ?case
              ...
              thus ?thesis
                proof
                  ...
                  then have "\<not>distinct (map (\<lambda>(u, v, _). (u,v)) (e # el)) \<or> 
                    (\<exists>u v. (u, v, 0) \<in> set (e # el))" unfolding ln_invar_def by auto
                  thus ?thesis unfolding ln_invar_def by simp
                next
                  assume "(\<exists>u c. (u, u, c) \<in> set el \<and> c > 0) \<or> 
                    (\<exists>u c. (u, s, c) \<in> set el \<and> c > 0) \<or> (\<exists>u c. (t, u, c) \<in> set el \<and> c > 0) \<or>
                    (\<exists>u v c1 c2. (u, v, c1) \<in> set el \<and> (v, u, c2) \<in> set el \<and> c1 > 0 \<and> c2 > 0)" 
                  
                  moreover {
                    assume "(\<exists>u c. (u, u, c) \<in> set el \<and> c > 0)"
                    then have "(\<exists>u c. (u, u, c) \<in> set (e # el) \<and> c > 0)" by auto
                  }
                  moreover {
                    assume "(\<exists>u c. (u, s, c) \<in> set el \<and> c > 0)"
                    then have "(\<exists>u c. (u, s, c) \<in> set (e # el) \<and> c > 0)" by auto
                  }
                  moreover {
                    assume "(\<exists>u c. (t, u, c) \<in> set el \<and> c > 0)"
                    then have "(\<exists>u c. (t, u, c) \<in> set (e # el) \<and> c > 0)" by auto
                  }
                  moreover {
                    assume "(\<exists>u v c1 c2. (u, v, c1) \<in> set el \<and> (v, u, c2) \<in> set el \<and> c1 > 0 \<and> c2 > 0)"
                    then have "(\<exists>u v c1 c2. (u, v, c1) \<in> set (e # el) \<and>
                      (v, u, c2) \<in> set (e # el) \<and> c1 > 0 \<and> c2 > 0)" by auto
                  }
          ...
          next
            ...
            have "(c' (u1, v1) > 0 \<or> c' (v1, u1) > 0 \<or> c1 = 0) \<or> 
              (c1 > 0 \<and> (u1 = v1 \<or> v1 = s \<or> u1 = t))"
              using obt1 obt2 obt3 False Cons.prems by (auto split:option.splits if_splits)
            moreover {
              assume "c1 = 0"
            ...
            moreover {
              assume "c' (v1, u1) > 0"
              then have "\<exists>c1'. (v1, u1, c1') \<in> set el \<and> c1' > 0" 
                using `c' = ln_\<alpha> el` unfolding ln_\<alpha>_def by (auto split:if_splits)
              then have "\<not>ln_invar (e # el) \<or> (\<exists>u v c1 c2.
                (u, v, c1) \<in> set (e # el) \<and> (v, u, c2) \<in> set (e # el) \<and> c1 > 0 \<and> c2 > 0)"
                proof (cases "c1 > 0")
                  case True
                    thus ?thesis using `\<exists>c1'. (v1, u1, c1') \<in> set el  \<and> c1' > 0` obt2 by auto
                next
                  case False
                    then have "\<not>ln_invar (e # el)" unfolding ln_invar_def using obt2 by auto
  ...
  lemma read'_correct: "read el s t = map_option pnet_\<alpha> (read' el s t)"
    ...
    by (auto simp: pnet_\<alpha>_def o_def ahm.correct ahs.correct ahm_ld_def split: option.splits)
      ...
      proof -
        ...
        }
        {
          ...
          moreover {
            {
              {
                ...
                then have "\<forall>v\<in>(Graph.V c). Graph.isReachable c s v" 
                  unfolding Graph.isReachable_def using Graph.isPath_alt_rtc1 by auto
              }
              moreover {
                ...
                then have "\<forall>v\<in>(Graph.V c). Graph.isReachable c v t"
                  unfolding Graph.isReachable_def using Graph.isPath_alt_rtc2 by auto
              }
            ...
            ultimately have "Network (pn_c x) s t" unfolding Network_def using asm1 fct asm7 by simp
          }
        ...
        proof
          ...
          then have "\<exists>v\<in>(Graph.V c). \<not>Graph.isReachable c v t"
            proof
              ...
              then have "\<exists>p. Graph.isPath c x p t" using Graph.isPath_alt_rtc2 by auto
              ...
              then have "x \<in> Graph.V c"
                ...
                next
                  case False
                    ...
                    then have "\<exists>v. c (x, v) > 0" by (metis neq0_conv prod.collapse)
            ...
            next
              ...
              have "\<forall>p. \<not>Graph.isPath c x p t"
                proof (rule ccontr)
                  assume "\<not>?thesis"
                  then obtain p where "Graph.isPath c x p t" by blast
                  thus "False" using Graph.isPath_alt `(x , t) \<notin> (Graph.E c)\<^sup>*` by auto
                qed
              then have "\<not>Graph.isReachable c x t" unfolding Graph.isReachable_def by auto
              thus ?thesis using o1 by auto
            qed
          moreover {
        ...
        next
          ...
          then have "\<exists>v\<in>(Graph.V c). \<not>Graph.isReachable c s v"
            proof
              ...
              then have "\<exists>p. Graph.isPath c s p x" using Graph.isPath_alt_rtc1 by auto
              ...
              then have "x \<in> Graph.V c"
                ...
                next
                  case False
                    ...
                    then have "\<exists>v. c (v, x) > 0" by (metis neq0_conv prod.collapse)
                    then have "x \<in> Graph.V c" unfolding Graph.V_def Graph.E_def by auto
                    thus ?thesis by simp
                qed
              thus ?thesis using o1 by auto
            next
              ...
              have "\<forall>p. \<not>Graph.isPath c s p x"
                proof (rule ccontr)
                  assume "\<not>?thesis"
                  then obtain p where "Graph.isPath c s p x" by blast
                  thus "False" using Graph.isPath_alt `(s, x) \<notin> (Graph.E c)\<^sup>*` by auto
                qed
              then have "\<not>Graph.isReachable c s x" unfolding Graph.isReachable_def by auto
              thus ?thesis using o1 by auto
            qed
          moreover {
            have "(el, c) \<in> ln_rel" using fct obt by simp
    ...
    proof -
      ...
      next {
        ...
        thus "False"
          ...
          next
            assume asm: "(\<exists>u c. (u, u, c) \<in> set el \<and> 0 < c) \<or> (\<exists>u c. (u, s, c) \<in> set el \<and> 0 < c) \<or>
              (\<exists>u c. (t, u, c) \<in> set el \<and> 0 < c) \<or> (\<exists>u v c1 c2. (u, v, c1) \<in> set el \<and>
              (v, u, c2) \<in> set el \<and> 0 < c1 \<and> 0 < c2)"
            
            moreover {
              assume "(\<exists>u c. (u, u, c) \<in> set el \<and> 0 < c)"
              then have "\<exists> u. ln_\<alpha> el (u, u) > 0" unfolding ln_\<alpha>_def apply (auto split:if_splits)
                by (metis (mono_tags, lifting) tfl_some)
              then have "False" using `Network (ln_\<alpha> el) s t` Network_def by simp
            }
            moreover {
              assume "(\<exists>u c. (u, s, c) \<in> set el \<and> 0 < c)"
              then have "\<exists> u. ln_\<alpha> el (u, s) > 0" unfolding ln_\<alpha>_def apply (auto split:if_splits)
                by (metis (mono_tags, lifting) tfl_some)
              then have "False" using `Network (ln_\<alpha> el) s t` Network_def by simp
            }
            moreover {
              assume "(\<exists>u c. (t, u, c) \<in> set el \<and> 0 < c)"
              then have "\<exists> u. ln_\<alpha> el (t, u) > 0" unfolding ln_\<alpha>_def apply (auto split:if_splits)
                by (metis (mono_tags, lifting) tfl_some)
              then have "False" using `Network (ln_\<alpha> el) s t` Network_def by simp
            }
            moreover {
              assume "(\<exists>u v c1 c2. (u, v, c1) \<in> set el \<and> (v, u, c2) \<in> set el \<and> 0 < c1 \<and> 0 < c2)"
              then obtain u v c1 c2 where o1: "(u, v, c1) \<in> set el \<and> (v, u, c2) \<in> set el 
                \<and> 0 < c1 \<and> 0 < c2" by blast
              then have "ln_\<alpha> el (u, v) > 0" unfolding ln_\<alpha>_def
                apply (auto split:if_splits) by (metis (no_types, lifting) someI)
              moreover have "ln_\<alpha> el (v, u) > 0" unfolding ln_\<alpha>_def using o1 
                apply (auto split:if_splits) by (metis (no_types, lifting) someI)
              ultimately have "\<not> (\<forall>u v. (ln_\<alpha> el) (u, v) \<noteq> 0 \<longrightarrow> (ln_\<alpha> el) (v, u) = 0)" by auto
              then have "False" using `Network (ln_\<alpha> el) s t` Network_def by auto
            }
