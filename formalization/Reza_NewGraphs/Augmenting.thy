...
begin
  ...
  begin
    definition augment :: "flow \<Rightarrow> flow"
      ("\<langle>\<up>/ _\<rangle>" 1000)
    where "\<langle>\<up> f'\<rangle> \<equiv> \<lambda>(u, v).
      if (u, v) \<in> E then
        f (u, v) + f' (u, v) - f' (v, u)
      else
        0"
  ...
  abbreviation NFlow_augment :: "graph \<Rightarrow> flow \<Rightarrow> flow \<Rightarrow> flow"
  ...
  context NFlow
  begin
    lemma augment_flow_presv_cap: "Flow cf s t f' \<Longrightarrow> \<forall>e. 0 \<le> \<langle>\<up> f'\<rangle> e \<and>  \<langle>\<up> f'\<rangle> e \<le> c e"
      proof -
        assume asm: "Flow cf s t f'"
        {
          ...
          moreover have "0 \<le> augment f' e"
            ...
            qed
          ultimately have "0 \<le> \<langle>\<up> f'\<rangle> e \<and>  \<langle>\<up> f'\<rangle> e \<le> c e" by blast   
        }
        thus ?thesis by simp
      qed       
  end
  ...
  context NFlow
  begin
    lemma augment_outflow_split: "Flow cf s t f' \<Longrightarrow> \<forall>v \<in> V. (\<Sum>e \<in> \<delta>\<^sup>- v. \<langle>\<up> f'\<rangle> e) =
      ...
      proof -
        assume asm: "Flow cf s t f'"
        {
          fix v
          assume asm_s: "v \<in> V"
          have "(\<Sum>e \<in> \<delta>\<^sup>- v. \<langle>\<up> f'\<rangle> e) = 
            (\<Sum>e\<in>{u |u. u \<in> V \<and> (v, u) \<in> E}. f (v, e)) +
            (\<Sum>e\<in>{u |u. u \<in> V \<and> (v, u) \<in> E}. f' (v, e))-
            (\<Sum>e\<in>{u |u. u \<in> V \<and> (v, u) \<in> E}. f' (e, v))"
            proof -
    ...
    lemma augment_inflow_split: "Flow cf s t f' \<Longrightarrow> \<forall>v \<in> V. (\<Sum>e \<in> \<delta>\<^sup>+ v. \<langle>\<up> f'\<rangle> e) =
  ...
  begin
    lemma augment_flow_presv_con: "Flow cf s t f' \<Longrightarrow> 
      \<forall>v \<in> V - {s, t}. (\<Sum>e \<in> \<delta>\<^sup>-v. \<langle>\<up> f'\<rangle> e) = (\<Sum>e \<in> \<delta>\<^sup>+ v. \<langle>\<up> f'\<rangle> e)"
      proof -
        assume asm: "Flow cf s t f'"
        {
          fix v
  ...
  context NFlow
  begin
    corollary augment_flow_presv: "Flow cf s t f' \<Longrightarrow> Flow c s t \<langle>\<up> f'\<rangle>"
      using augment_flow_presv_cap augment_flow_presv_con Flow_def by auto
       
    lemma augment_flow_value: "Flow cf s t f' \<Longrightarrow> \<lbrace>c, s \<parallel>\<^sub>F |\<langle>\<up> f'\<rangle>|\<rbrace> = val + \<lbrace>cf, s \<parallel>\<^sub>F |f'|\<rbrace>"
      proof -
