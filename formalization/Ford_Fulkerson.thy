...
begin
  ...
  context FoFu
  begin
    lemma fofu_I_II: "isMaxFlow c s t f \<Longrightarrow> \<not> (\<exists> p. \<langle>\<Rightarrow>\<^sup>A p\<rangle>)"
    ...
    lemma fofu_II_III: "\<not> (\<exists> p.\<langle>\<Rightarrow>\<^sup>A p\<rangle>) \<Longrightarrow> \<exists>k'. NCut c s t k' \<and> val = \<lbrace>c \<parallel>\<^sub>N\<^sub>C Cap k'\<rbrace>" 
      proof -
        ...
        {
          have "Graph.reachableNodes cf s \<subseteq> V"  using Graph.reachable_ss_V s_node resV_netV by auto
            ...
            have "f e = c e"
              proof (rule ccontr)
                ...
                moreover obtain p where "Graph.isPath cf s p u" 
                  using fct_s Graph.reachableNodes_def Graph.isReachable_def by auto
                ultimately have "Graph.isPath cf s (p @ [(u, v)]) v" 
                  using Graph.isPath_append by auto
                thus "False" using fct_s Graph.reachableNodes_def Graph.isReachable_def by auto
              qed
          }
  ...
  context NFlow
  begin
    lemma maxFlow_iff_noAugPath: "\<not> (\<exists> p. \<langle>\<Rightarrow>\<^sup>A p\<rangle>) \<longleftrightarrow> isMaxFlow c s t f"
      proof -
        let ?S = "{s}"
  ...
  begin
    lemma maxFlow_minCut: "\<lbrakk>isMaxFlow c s t f; isMinCut c s t k\<rbrakk> \<Longrightarrow> val = cap"
      proof -
        ...
        obtain k' where obt1: "NCut c s t k'" and obt2: "val = \<lbrace>c \<parallel>\<^sub>N\<^sub>C Cap k'\<rbrace>" using f2 by blast
        {
          fix k''
        ...
        then have "\<lbrace>c \<parallel>\<^sub>N\<^sub>C Cap k'\<rbrace> = cap" using asm2 isMinCut_def by force
