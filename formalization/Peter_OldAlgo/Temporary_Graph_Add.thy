...
begin
  ...
  begin
    ...
    abbreviation (input) conn :: "node \<Rightarrow> node \<Rightarrow> bool"
      -- \<open>Predicate that two nodes are connected\<close> (* TODO: Rename isReachable to connected! *)
      where "conn \<equiv> isReachable"
    ...
    lemma conn_alt: "conn v v' = (\<exists>d. dist v d v')"
      by (auto simp: dist_def isReachable_def)
    ...
    lemma
      shows connI[intro]: "dist v d v' \<Longrightarrow> conn v v'"
        and connI_id[intro!, simp]: "conn v v"
        and connI_succ: "conn v v' \<Longrightarrow> (v',v'') \<in> E \<Longrightarrow> conn v v''"
      by (auto simp: dist_def isReachable_def intro: isPath_append_edge)
    ...
    lemma min_dist_succ: 
      "\<lbrakk> conn v v'; (v',v'') \<in> E \<rbrakk> \<Longrightarrow> min_dist v v'' \<le> Suc (min_dist v v') "
    ...
    lemma min_dist_suc:
      ...
      shows "\<exists>v''. conn v v'' \<and> (v'',v') \<in> E \<and> min_dist v v'' = d"
    proof -
    ...
    lemma shortestPath_level_edge:  
      ...
      shows 
        "conn s u" "conn u v" "conn v t" and
    ...
    proof -  
      ...
      from DISTS show "conn s u" "conn u v" "conn v t" by auto
