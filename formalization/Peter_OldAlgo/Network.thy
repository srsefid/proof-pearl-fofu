...
begin
  ...
  locale Network = Graph +
    ...
    assumes no_self_loop : "\<forall>u. c(u, u) = 0"
    assumes no_incoming_s: "\<forall>u. c (u, s) = 0"
    assumes no_outgoing_t: "\<forall>u. c (t, u) = 0"
    assumes no_parallel_edge: "\<forall>u v. c (u, v) \<noteq> 0 \<longrightarrow> c (v, u) = 0"
    assumes nodes_on_st_path: "\<forall>v \<in> V. isReachable s v \<and> isReachable v t"
    assumes finite_reachable: "finite (reachable s)"
  begin
    lemma no_parallel_edge': "(u,v)\<in>E \<Longrightarrow> (v,u)\<notin>E"
      using no_parallel_edge by (simp add: E_def)

    lemma V_ss_reachable : "V \<subseteq> reachable s"
      proof
        fix x
        assume "x \<in> V"
        thus "x \<in> reachable s"
          unfolding reachable_def by (simp add: s_node nodes_on_st_path)
      qed
    
    lemma reachable_is_V[simp]: "reachable s = V"
  ...
  begin
    definition "cap \<equiv> (\<Sum>e \<in> outgoing' k. c e)"
  end
  
  definition isMinCut :: "graph \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> cut \<Rightarrow> bool"  where
    "isMinCut c s t k \<equiv> NCut c s t k \<and>
  ...
  begin      
    lemma no_inflow_s: "\<forall>e \<in> incoming s. f e = 0" (is ?thesis)
      proof (rule ccontr)
        ...
        then have "c e \<noteq> 0" using capacity_cons by (metis le_0_eq)
  ...
  locale FoFu = NCut + NFlow
    begin  
    definition "netFlow \<equiv> (\<Sum>e \<in> outgoing' k. f e) - (\<Sum>e \<in> incoming' k. f e)"
    ...
    lemma weak_duality: "val \<le> cap"
      proof -
        have "(\<Sum>e \<in> outgoing' k. f e) \<le> (\<Sum>e \<in> outgoing' k. c e)" (is "?L \<le> ?R") 
          using capacity_cons by (metis setsum_mono)
        then have "(\<Sum>e \<in> outgoing' k. f e) \<le> cap" unfolding cap_def  by simp
        moreover have "val \<le> (\<Sum>e \<in> outgoing' k. f e)" using flow_value netFlow_def by auto
      ...
      proof -
        ...
        then have fct: "FoFu c s t ?K f" unfolding FoFu_def using NFlow_axioms by auto
        {
          ...
          moreover have "outgoing' ?K = incoming t"
            unfolding outgoing'_def incoming_def E_def V_def using no_self_loop by auto
          moreover have "incoming' ?K = outgoing t"
            unfolding incoming'_def outgoing_def E_def V_def using no_self_loop by auto
  ...
  definition isMaxFlow :: "graph \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> flow \<Rightarrow> bool" where
     "isMaxFlow c s t f \<equiv> NFlow c s t f \<and> 
