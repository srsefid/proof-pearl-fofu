...
imports Refine_Add_Fofu Graph_Impl Temporary_Graph_Add "~~/src/HOL/Library/Rewrite"
...
begin
  ...
  begin  
    ...
    where
      "\<And>d. Vd d \<equiv> {v. conn src v \<and> ndist v = d}"
    ...
    lemma in_Vd_conv: "v\<in>Vd d \<longleftrightarrow> conn src v \<and> ndist v = d"
      by (auto simp: Vd_def)
    ...
    lemma Vd_succ: 
      ...
      shows "v\<in>Vd (Suc d)"
      using assms
      by (metis connI_succ in_Vd_conv le_SucE min_dist_succ)
  ...
  lemma (in nf_invar) invar_C_ss_V: "u\<in>C \<Longrightarrow> u\<in>V"  
    apply (drule CVdI)
    apply (auto simp: in_Vd_conv isReachableV)
    done
  ...
  definition "bfs_spec src dst r \<equiv> (
    case r of None \<Rightarrow> \<not> conn src dst
            | Some (d,PRED) \<Rightarrow> conn src dst \<and> min_dist src dst = d \<and> valid_PRED c src PRED \<and> dst\<in>dom PRED)"
  ...
  lemma less_than_bool_iff[simp]:
    "(x,y)\<in>less_than_bool \<longleftrightarrow> x=True \<and> y=False"  
  ...
  proof -
    ...
    hence "min_dist src u = d" "u\<in>V" by (auto simp: in_Vd_conv isReachableV)
    thus "d\<le>max_dist src" 
      unfolding max_dist_def by auto
  qed    

  lemma (in nf_invar) Vd_ss_V: "Vd d \<subseteq> V"
    by (auto simp: Vd_def isReachableV)

  lemma (in nf_invar) finite_C[simp, intro!]: "finite C"
    using C_ss FIN_V Vd_ss_V by (blast intro: finite_subset)
  
  lemma (in nf_invar) finite_succ: "u\<in>C \<Longrightarrow> finite (E``{u})"  
    using C_ss Vd_ss_V finite_subset succ_in_V by blast
  ...
  context Graph begin
    ...
    lemma bfs_correct:
      ...
      shows "bfs src dst \<le> SPEC (\<lambda>None \<Rightarrow> \<not>conn src dst | Some p \<Rightarrow> shortestPath src p dst)"
      unfolding bfs_def
      apply (refine_vcg
      ...
      using assms
      apply (vc_solve simp: bfs_spec_def shortestPath_min_dist_def isSimplePath_def)
      done      
  end
  ...
  context Finite_Graph begin
    ...
    theorem
      ...
      shows "bfs src dst \<le> (spec p. 
        case p of None \<Rightarrow> \<not>conn src dst | Some p \<Rightarrow> shortestPath src p dst)"
      unfolding bfs_def
      apply (refine_vcg
        pre_bfs_correct[THEN order_trans]
        valid_PRED.extract_rpath_correct[THEN order_trans]
