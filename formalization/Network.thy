...
begin
  ...
  where "isMaxFlow c s t f \<equiv> NFlow c s t f \<and> 
    (\<forall>f'. NFlow c s t f' \<longrightarrow> \<lbrace>c, s \<parallel>\<^sub>F |f'|\<rbrace> \<le> \<lbrace>c, s \<parallel>\<^sub>F |f|\<rbrace>)"
  (*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*)
  (*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*)
  (*^^^^^^^^^^^^^^^^^^^^^^^END^^^^^^^^^^^^^^^^^^^^^^^^*)
  ...
  context NFlow 
  begin      
    lemma no_inflow_s: "\<forall>e \<in> \<delta>\<^sup>+ s. f e = 0" (is ?thesis)
      proof (rule ccontr)
    ...
    lemma no_outflow_t: "\<forall>e \<in> \<delta>\<^sup>- t. f e = 0"
      proof (rule ccontr)
    ...
    lemma val_alt: "val = (\<Sum>e \<in> \<delta>\<^sup>- s. f e)"
  ...
  context NFlow
  begin
    lemma inflow_t_outflow_s: "(\<Sum>e \<in> \<delta>\<^sup>+ t. f e) = (\<Sum>e \<in> \<delta>\<^sup>- s. f e)"
      proof -
        let ?K = "V - {t}"
